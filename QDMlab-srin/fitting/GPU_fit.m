...
idx = ~cellfun('isempty', regexpi({dataFiles.name}, 'run_[0-9]{5}\.mat$','match')); %%srin need our own formatting of filename
...
for fileNum=startN:1:endN
    ...
    % read data & header
    expData = load(fullfile(dataFolder, dataFile)); %%srin need to change this to take gWide from the .mat
    ...
    for nRes = 1:2 %%srin while not necessary, maybe we keep the loop because it may allow for multiple b fields at once? idk
        side = sides{nRes}; %%like we should have a folder with multiple files of B or T, and then it fits them all at once? replace nres and sides with B and T? or what about P also? maybe just 1 index
    ...
    if any(strcmp(fieldnames( fits.(['right' pol])), 'contrastB')) %%srin B and C are for hyperfine peaks, but this structure should be adapted for fitting multiple NV resonances
    ...
    ResSum = (Resonance2 + Resonance1)/2; %%srin add a line to extract pressure
      
    if kwargs.quadBgSub %%srin yeah don't do this, it's subtracting a quadratic background from the E map, which...why? However, this is where to include electric/stress subtraction
