function plot_point_pages(max_points, points, name)
    disp(name)
    figure;
    ...
    for k = 1:num_pages
        ...
        title([name 'Points in Bucket ,' num2str(k)]);
        axis equal;
        axis tight;
        hold off;
    end
    ...
    hold on; 
    [~, cols] = size(points(:, :, 1));
    linebreaks = cols:cols:numel(points(:, :, 1));
    for i=1:size(points, 3) 
        move = points(:, :, i);
        move(move == 0) = NaN; 
        % move = imgaussfilt(move, 3);
        plot(move(:), "o", "DisplayName", ['Slice ' num2str(i)]); 
    end 
    axis tight;
    xline(linebreaks, ':', "HandleVisibility", 'off'); 
    legend();
end
...
minsi = zeros(max_points, 1);
minsi2 = last_el;
clustered_points = zeros(size(points));
clustered_points2 = zeros(size(points));
for i=1:size(points, 1)
    for j=1:size(points, 2)
        locs = squeeze(points(i, j, :));
        dists = pdist2(locs, minsi);
        pairs = matchpairs(dists, 3e8);
        clustered_points(i, j, pairs(:, 1)) = locs(pairs(:, 2));
        if i == 90
            disp([i, j, pairs(:, 1)'])
            disp([minsi, locs])
            disp(pairs)
        end
        minsi(pairs(:, 2)) = locs(pairs(:, 2));
    end
end
...
for i=size(points, 1):-1:1
    for j=size(points, 2):-1:1
        locs = squeeze(points(i, j, :));
        dists = pdist2(locs, minsi2);
        pairs = matchpairs(dists, 3e8);
        clustered_points2(i, j, pairs(:, 2)) = locs(pairs(:, 1));
        minsi2 = locs;
    end
end

plot_point_pages(max_points, points, "Original")
plot_point_pages(max_points, clustered_points, "Forwards")
% plot_point_pages(max_points, clustered_points2, "Backwards")
...
for k = (length(all_points) - length(mins2)):-1:1 % for each peak after the first pixel
    ...
    % disp("Start")
    % disp([dist, mins2, curr_dists])
    dist(dist >= curr_dists) = inf;
    % disp([dist; all_points(k)])
...
% disp("old")
% plot_point_pages(max_points, points, "Old")

% disp("new")
% plot_point_pages(max_points, sorted_bucketed_points, "new")
close(f)
