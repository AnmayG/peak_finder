function [Ypk,Xpk,Wpk,Ppk] = findpeaks_custom(Yin,varargin)
...
if hasTimeInfo
  ...
  if FsSupplied
    ...
    % X
    Xin1 = varargin{1};
    % validateattributes(Xin1,{'numeric','datetime'},{'vector'},'findpeaks','X');
    if ~isInMATLAB
        coder.internal.errorIf(isdatetime(Xin1),'signal:findpeaks:DatetimeInputsNotSupported');
    end
    if isnumeric(Xin1)
      % validateattributes(Xin1,{'numeric'},{'real','finite','increasing'},'findpeaks','X');
      Xin = double(Xin1);
    else % isdatetime(Xin)
      % validateattributes(seconds(Xin1-Xin1(1)),{'double'},{'real','finite','increasing'},'findpeaks','X');
      Xin = Xin1;
    end
...
if isInMATLAB
    ...
    validParams = {'MinPeakHeight', 'MinPeakProminence', 'MinPeakWidth', 'MaxPeakWidth', ...
                   'MinPeakDistance', 'Threshold', 'NPeaks', 'SortStr', 'Annotate', 'WidthReference'};
    
    % Iterate through varargin and assign values to corresponding parameters
    for i = startArg:2:length(varargin)
